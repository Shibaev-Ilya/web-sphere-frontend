...
* ```bem create block-name``` - добавить БЭМ-блок
...
Чтобы вручную не создавать соответствующие папку и файлы, достаточно в консоли прописать следующую команду: ```bem create my-block``` - для создания БЭМ-блока в ```src/block/modules```, где ```my-block``` - имя БЭМ-блока.
...
### Библиотеки 
  * слайдер [Swiper](https://swiperjs.com/) для установки в проект ```yarn add swiper```
